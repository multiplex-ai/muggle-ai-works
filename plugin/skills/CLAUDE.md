...
| `sonnet` | `muggle-pr-visual-walkthrough`, `muggle-test-prepare`, `muggle-test-regenerate-missing` | Multi-step orchestration with light judgment, short of deep reasoning: assemble run data and build a PR section with fit-vs-overflow handling; diagnose service health and decide a clean restart; scan, filter, bulk-dispatch, and classify per-item failures into buckets. More moving parts than a router, but each step is well-defined. |
