...
**Where a pin must actually bite, put it on an agent.** A SKILL.md `model:` is applied only when the skill starts a fresh session (slash command in a new conversation); invoked mid-session — via the Skill tool, a queued prompt, or a nested skill — the session model keeps running (verified 2026-07-23, CLI 2.1.211: fresh-session probe ran 100% on the pinned model; mid-session invocations ran on the session model). Agent frontmatter `model:` is harness-applied on every dispatch. Hence the pattern for pin-critical skills: the SKILL.md stays the entry point and resolves all user interaction, then dispatches a `plugin/agents/*` agent carrying the real pin — `muggle-pr-followup` (watcher agent, haiku), `muggle-pr-visual-walkthrough` (`visual-walkthrough-builder`, sonnet), `muggle-test-prepare` (`test-prepare-runner`, opus). Agents never ask the user; an unresolved decision comes back as `needs-input:` for the skill to resolve.
