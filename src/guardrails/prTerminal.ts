...
// model chose to run.
function terminalProvenance(input: HookInput): {
  acceptsForgeLine: boolean;
  acceptsMonitorLine: boolean;
} {
...
export function detectPrTerminal(input: HookInput): PrTerminalEvent | null {
  ...
  const monitorMatch = provenance.acceptsMonitorLine
    ? haystack.match(PR_MONITOR_TERMINAL_LINE)
    : null;
  if (monitorMatch) {
    return {
