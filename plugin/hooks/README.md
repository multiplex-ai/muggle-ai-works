...
The nudge is **owner-scoped**, and the scope is the point. Each slot records the session that armed it in `owner.json`; the hook reads `session_id` off the event payload and counts slots this session owns separately from the rest. Only the owned count is framed as recoverable work — those are watchers this session armed and may have lost. Slots owned by another session (or carrying no `owner.json` at all) are reported as orphans with the `adopt <slug>` command and an explicit instruction not to act on them unprompted, because re-arming a stranger's PR hands review work to a session with no context for it. Un-scoped, this nudge was the mechanism by which every fresh session inherited every open watcher on the machine.

Two exclusions keep the count honest: a `*.stopped` dir is the owner's kill switch and is never counted (it keeps its `prs.json`, so the name is the only signal), and a slot with `result.md` is already finalized. If the payload carries no `session_id` the hook fails closed — every slot reads as foreign, so it reports and never claims. Nothing owned and nothing orphaned → it emits nothing. A pure directory scan (no `gh`, no writes), so it's cheap enough for every session start.
