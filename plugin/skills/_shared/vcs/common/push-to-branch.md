...
**Signing gate — never push unsigned commits.** Check what is about to leave: `git -C <repo-path> log --format='%G?' origin/<head-ref-name>..HEAD`. Any `N` → do not push; follow the resolved provider's signed-commits recipe — `github` → [`../github/signed-commits.md`](../github/signed-commits.md) (create the commits server-signed), `gitlab` → [`../gitlab/signed-commits.md`](../gitlab/signed-commits.md) (stop and escalate — no server-side signing). Never disable signing to make a push go through.
