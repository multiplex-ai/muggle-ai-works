...
3. **Mark the slot stopped.** Rename `~/.muggle-ai/muggle-do/sessions/<slug>/` → `<slug>.stopped/`. This single rename is the load-bearing act: the tick absorb keys on it, [`reconcile.md`](reconcile.md) skips `*.stopped` dirs, and the out-of-session watchdog skips them too — so no recovery path of any kind re-arms the slot.
...
- **Nothing revives a stopped slot.** Tick, reconcile, auto-track, and the watchdog all treat `*.stopped` as invisible. Only the owner resumes one (rename back + re-arm), and only bootstrap overrides the global kill file.
