...
9. **Branch hygiene signals** — collect facts now; the questions below in 11/12 will use them:
   - Is the current checkout already a worktree? `git -C <repo> rev-parse --is-inside-work-tree` plus inspect `git -C <repo> worktree list`. Feeds the worktree question — see [`../_shared/use-worktrees.md`](../_shared/use-worktrees.md).
   - How many commits behind `origin/<default>` is the current branch? `git -C <repo> fetch origin && git -C <repo> rev-list --count "HEAD..origin/$(git -C <repo> symbolic-ref refs/remotes/origin/HEAD --short | sed 's|origin/||')"`. Feeds the rebase question — see [`../_shared/rebase-before-e2e.md`](../_shared/rebase-before-e2e.md).
...
11. **Worktree?** — only if the current checkout is **not** already a worktree and the task is more than a trivial edit. Per [`../_shared/use-worktrees.md`](../_shared/use-worktrees.md): "Create a worktree for this change (recommended)" / "Work directly in the current checkout". If the user picks worktree, defer creation to `superpowers:using-git-worktrees` after pre-flight closes.
12. **Rebase onto `origin/<default>`?** — only if the silent-detection `behind` count is `> 0`. Per [`../_shared/rebase-before-e2e.md`](../_shared/rebase-before-e2e.md): "Rebase first (recommended — N commits behind)" / "Proceed against current branch". Record the choice in `state.md`; the E2E stage re-checks before launching the dev server.
