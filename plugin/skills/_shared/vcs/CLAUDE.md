...
Provider-agnostic seam for the watcher and dev-cycle skills. A caller resolves one provider token via [`detect-vcs.md`](detect-vcs.md) — `github` (`gh`) or `gitlab` (`glab`) — then loads the matching provider's recipe for each agnostic step. The two recipe sets mirror each other call-for-call; callers branch on the token, never fork procedure. Recipes with no provider-specific procedure live once in `common/` and are indexed by both TOCs.
...
- `common/` — provider-agnostic recipes (`push-to-branch`, `verify-working-tree`), indexed by both provider TOCs.
