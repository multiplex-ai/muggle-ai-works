...
The delete every tick uses to stop its own loop: the stale-fire guard and terminal unschedule ([`finalize.md`](finalize.md) Step 4), and each single-thread "stop this watcher" before a `/muggle-do` dispatch ([`contract.md`](contract.md) Steps 4–6). A blocked watcher does **not** cancel its cron — it keeps the `1m` cadence and only reminds ([`contract.md`](contract.md) Steps 2.5, 7).
