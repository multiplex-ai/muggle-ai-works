...
After reporting results, ask the user if they want to attach a visual walkthrough — a markdown comment with per-test-case dashboard links and screenshots — to the current branch's open PR. This is a separate, reusable skill: `muggle-pr-visual-walkthrough`.
...
### 9b: Invoke the shared skill

If the user chooses "Yes, post to PR", invoke the `muggle-pr-visual-walkthrough` skill via the `Skill` tool. That skill handles:

- Finding the PR via `gh pr view` (or offering to create one if none exists)
- Building the walkthrough markdown comment
- Posting via `gh pr comment` (or embedding in `gh pr create`)
- Confirming to the user with the PR link

**Required context for the invocation** — make sure these are present in the conversation before handing off:

- `projectId`
- Per test case: `title`, `testCaseId`, `status`, `steps`, `duration`, `viewUrl`, and failure details / artifact paths on failure

The `muggle-pr-visual-walkthrough` skill will not run tests or publish results — it only formats and posts what's already in context.
