...
| `PostToolUse` (Bash) | `guardrail-record-comment-replies.sh` | record | an unresolved-thread fetch named review comments still awaiting an answer (classified by the `<!-- muggle-do:bot -->` marker, never the author login), a threaded reply answered one, a push landed the change acting on them, or a `MUGGLE_REPLY_SKIP` marker deferred a comment | — | set `commentRepliesOwed` / `commentRepliesPosted` / `reviewWorkPushed` / `commentReplySkipped` |
...
| `Stop` | `guardrail-comment-reply-gate.sh` | **enforce** | a review round pushed its change this session and a thread it pulled in still carries no loop-marked reply | — | **block** the turn until each comment gets its threaded reply per `do/per-comment-replies.md` or a `MUGGLE_REPLY_SKIP: <comment-id> <reason>` marker clears that comment (releases after 3 blocks). Reading reviews is not acting on them — with no push recorded the gate stays silent, so an ambiguous-only round that escalates can still end its turn |
