...
- [`watch-mode.md`](watch-mode.md) — how a watcher is armed: the `Monitor`-hosted [`poll.sh`](poll.sh) detector, the line grammar it emits, and how an emitted line becomes a `contract.md` tick via the [`pr-watcher`](../../agents/pr-watcher.md) agent.
