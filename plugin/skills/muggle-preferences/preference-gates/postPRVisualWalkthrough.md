...
**PR detection (mandatory before any picker).** Run `gh pr view --json number,title,url 2>/dev/null` first. The result picks Case A or B. Substitute `{prNumber}`, `{prTitle}` into prompts.
...
Saved value covers this case — `always` includes auto-creating the PR. Picker 1 only runs when the gate is `ask`.

**Picker 1** — header `No PR yet`, question `"This branch has no open PR. Create one and post the walkthrough, or skip?"`
- `Create a PR and post` — `I'll open a PR for this branch, then attach the walkthrough.` → run PR-creation flow, then post. → `always`
