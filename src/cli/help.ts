...
export function getHelpGuidance(): string {
  const lines = [
    ...
    "",
    `  ${colorize("Inspecting Runs:", COLORS.bold)}`,
    `    ${cmd("muggle run-view")}         Show a run's steps beside the frames they produced`,
    `    ${cmd("muggle run-view --html")}  Render those frames as a self-contained page`,
    "",
    ...
    "    muggle-local-run-result-get, muggle-local-run-steps-get, muggle-local-check-status, etc.",
    "",
    ...
    `    ${path("sessions/")}            Test execution sessions (read one with ${cmd("muggle run-view")})`,
